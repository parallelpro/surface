...
! ***********************************************************************
      ...
      integer function extras_check_model(id)
         ...
         ! protostar to rgb numax=13
         if ((s% center_h1 < 0.0001) .and. (s%nu_max <= 10)) then
            !!!(s%delta_Pg <= (0.13 * s%delta_nu / (s%nu_max**2.0) * 1.e6)) .and. (s%delta_Pg .ne. 0.) ) then
            !!! stop when star hydrogen mass drops to specified level
            extras_check_model = terminate
            termination_code_str(t_xtra1) = 'have reached RGB numax = 10 muHz.'
            return
         end if
      ...
      integer function extras_finish_step(id)
         ...
         ! ZAMS to RGB tip (Dnu = 1 muHz)
         if (((s%center_h1 < 0.0001) .or. ((s%L_nuc_burn_total/s%L_surf)>=0.99) ) &
                .and. (s%nu_max>=10.) .and. (.not.(s%helium_ignition))) then
            ! save profile models
